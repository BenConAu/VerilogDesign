...
bool StatementNode::PreProcessNodeImpl(OutputContext* pContext)
{
    ...
    if (pModule->GetFunctionExpandType() == FunctionExpandType::StageNonblocking)
    {
        // Outputs of stages are basically like function calls if we have elected
        // to expand the stages all out to be nonblocking assignments, so process 
        // them the same way that we would functions.
